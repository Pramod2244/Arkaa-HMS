...
import {
  ...
  Factory, Home, ChevronRight as ChevronRightIcon, ChevronRight,
} from "lucide-react";
import { format } from "date-fns";
import { ManufacturerFormDrawer } from "@/components/pharmacy/manufacturer-form-drawer";
...
function Breadcrumb() {
  return (
    <nav className="flex items-center gap-2 text-sm text-slate-500 mb-4">
      ...
      <ChevronRightIcon className="h-4 w-4" />
      <span className="text-slate-900 font-medium">Manufacturers</span>
    </nav>
  );
}
...
export default function ManufacturersPage() {
  ...
  const [selected, setSelected] = useState<ManufacturerRecord | null>(null);
  ...
  const fetchData = useCallback(async (pageCursor?: string | null) => {
    ...
    try {
      ...
      const res = await fetch(`/api/pharmacy/masters/manufacturers?${params}`);
  ...
  useEffect(() => {
    const t = setTimeout(() => { setCursorHistory([]); setCursor(null); }, 300);
  ...
  const handleDelete = async (rec: ManufacturerRecord) => {
    if (!confirm(`Delete "${rec.name}"?`)) return;
    try {
      const res = await fetch(`/api/pharmacy/masters/manufacturers?id=${rec.id}`, { method: "DELETE" });
      const result = await res.json();
  ...
  const columns = [
    { key: "code" as keyof ManufacturerRecord, header: "Code" },
    { key: "name" as keyof ManufacturerRecord, header: "Name" },
    { key: "licenseNumber" as keyof ManufacturerRecord, header: "License", render: (v: string | null) => <span className="text-slate-500">{v || "-"}</span> },
    { key: "contactNumber" as keyof ManufacturerRecord, header: "Contact", render: (v: string | null) => <span className="text-slate-500">{v || "-"}</span> },
    {
      key: "status" as keyof ManufacturerRecord, header: "Status",
      render: (v: "ACTIVE" | "INACTIVE") => (
        <Badge className={v === "ACTIVE" ? "bg-green-100 text-green-800" : "bg-slate-100 text-slate-600"}>{v}</Badge>
      ),
    },
    {
      key: "updatedAt" as keyof ManufacturerRecord, header: "Updated",
      render: (v: string) => <span className="text-slate-500 text-sm">{format(new Date(v), "PP")}</span>,
    },
    {
      key: "id" as keyof ManufacturerRecord, header: "Actions",
      render: (_v: string, row: ManufacturerRecord) => (
        <DropdownMenu>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-100 rounded-lg"><Factory className="h-6 w-6 text-blue-600" /></div>
            <div>
              <h1 className="text-2xl font-semibold text-slate-900">Manufacturers</h1>
              <p className="text-sm text-slate-500">Manage product manufacturers</p>
            </div>
          </div>
          <Button onClick={() => { setSelected(null); setIsDrawerOpen(true); }}><Plus className="h-4 w-4 mr-2" />Add Manufacturer</Button>
        </div>
        ...
        <GlassCard className="p-0 overflow-hidden">
          <DataTable columns={columns} data={records} loading={isLoading} emptyMessage="No manufacturers found" />
