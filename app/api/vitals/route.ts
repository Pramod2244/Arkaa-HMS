...
import { createVitals, getVitalsByVisitId, getVitalsById, updateVitals, deleteVitals } from "@/lib/services/vitals";
...
export async function POST(request: NextRequest) {
  try {
    ...
    requirePermission(session, "VITAL_CREATE");

    const body = await request.json();
    const validatedData = VitalsSchema.parse(body);

    const vitals = await createVitals(validatedData, session.tenantId, session.userId);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const vitals = await getVitalsByVisitId(visitId, session.tenantId);
