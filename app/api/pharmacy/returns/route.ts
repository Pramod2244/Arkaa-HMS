...
export async function GET(request: NextRequest) {
  try {
    ...
    const queryData = {
      ...
      returnType: searchParams.get("returnType") || "ALL",
      saleId: searchParams.get("saleId") || undefined,
      limit: searchParams.get("limit") ? parseInt(searchParams.get("limit")!) : 20,
      cursor: searchParams.get("cursor") || undefined,
    };

    const validatedQuery = ReturnQuerySchema.parse(queryData);

    const result = await getReturnsCursor(session.tenantId, {
      search: validatedQuery.search,
      status: validatedQuery.status === "ALL" ? undefined : validatedQuery.status,
      returnType: validatedQuery.returnType === "ALL" ? undefined : validatedQuery.returnType,
      saleId: validatedQuery.saleId,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!session.permissions.includes("PHARMACY_RETURN_CREATE")) {
      return NextResponse.json(
        { success: false, errorCode: "FORBIDDEN", message: "Access denied" },
        { status: 403 }
      );
    ...
    const result = await createReturn(session.tenantId, session.userId, validatedData);

    return NextResponse.json(
      { success: true, data: result, message: "Return created successfully" },
      { status: 201 }
    );
  } catch (error) {
